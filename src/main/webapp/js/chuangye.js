/**
 * Created by wd on 2017/4/21.
 */
window.chuangye = window.chuangye || {};
//var userNumber = userSession.userNumber;
var queryParams = {number: userNumber,navContent:$('#nav-content').text()};
var $mainTable = $('#chuangye_table');
var $checkTable =$('#check_table');
var uploadFile='attachNameChuangye';
var mainModal = 'chuangye-modal';
var mainModalForm = 'chuangye-form';
var attachNameDownloadId = 'attacheChuangyeDownLoad';
var modalTitle = 'chuangyeModelTitle';
chuangye.init = function () {
    init_main_table();
    ...
    $('#'+mainModal).on('shown.bs.modal', function () {
        ...
        if($('#'+modalTitle).html().length==2) {
            $('#'+modalTitle).html($('#'+modalTitle).html() + "创业孵化");
        }
    ...
    $('#btn-modal-save').click(function () {
        if ($('#cnNameChuangye').val() == '') {
            layer.msg('公司名称不可以为空!', {icon: 5})
            return;
        }
        if ($('#achieveTypeChuangye').val() == '') {
            layer.msg('成果类型不可以为空!', {icon: 5})
            return;
        }
        if ($('#authorRankChuangye').val() == '') {
            layer.msg('作者排序不可以为空!', {icon: 5})
            return;
        }
        if ($('#companyAddrChuangye').val() == '') {
            layer.msg('注册地点不可以为空!', {icon: 5})
            return;
        }
        if ($('#companyNumberChuangye').val() == '') {
            layer.msg('营业执照不可以为空!', {icon: 5})
            return;
        }
        if ($('#companyTimeChuangye').val() == '') {
            layer.msg('创建日期不可以为空!', {icon: 5})
            return;
        }
        if ($('#belongAcademyChuangye').val() == '') {
...
function saveInfo() {
    var params = {
        cnName: $('#cnNameChuangye').val().trim(),
        companyAddr: $('#companyAddrChuangye').val().trim(),
        companyNumber: $('#companyNumberChuangye').val().trim()
    }
}

function init_add_modal_select() {
    init_select('achieveTypeChuangye', selectClassifyTypeUrl, {pidName: pidName}, navContent);
    init_select('authorRankChuangye', selectAuthorRankUrl, {pidName: pidName}, null);
    init_select('belongAcademyChuangye', selectAcademyAllUrl, null, userAcademyName);

}

function init_edit_modal_select(hrs) {
    init_select('achieveTypeChuangye', selectClassifyTypeUrl, {pidName: pidName}, hrs[0].achieveType);
    init_select('authorRankChuangye', selectAuthorRankUrl, {pidName: pidName}, hrs[0].authorRank);
    init_select('belongAcademyChuangye', selectAcademyAllUrl, null, hrs[0].belongAcademy);

}

function init_edit_modal_input(hrs) {
    $('#cnNameChuangye').val(hrs[0].cnName);
    $('#chuangye-id').val(hrs[0].id);
    $('#companyAddrChuangye').val(hrs[0].companyAddr);
    $('#companyNumberChuangye').val(hrs[0].companyNumber);
    $('#companyTimeChuangye').val(hrs[0].companyTime);
    $('#commentChuangye').val(hrs[0].comment);
    //TODO 加上传文件 的信息
}
...
function init_main_table() {
    $mainTable.bootstrapTable({
        ...
        columns: [{
            ...
            {field: 'companyAddr', title: '注册地点'},
            {field: 'companyNumber', title: '营业执照'},
            {field: 'companyTime', title: '创建日期'},
