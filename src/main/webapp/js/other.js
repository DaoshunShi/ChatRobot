...
var $mainTable = $('#other_table');
var $checkTable =$('#check_table');
var uploadFile='attachNameOther';
var mainModal = 'other-modal';
var mainModalForm = 'other-form';
var attachNameDownloadId = 'attacheOtherDownLoad';
var modalTitle = 'otherModelTitle'
...
other.init = function () {
    ...
    $('#'+mainModal).on('shown.bs.modal', function () {
        ...
        if($('#'+modalTitle).html().length==2) {
            $('#'+modalTitle).html($('#'+modalTitle).html() + "其他");
        }
    ...
    $('#btn-modal-save').click(function () {
        ...
        if ($('#achieveTypeOther').val() == '') {
            layer.msg('成果类型不可以为空!', {icon: 5})
            return;
        }
        if ($('#authorRankOther').val() == '') {
            layer.msg('作者排序不可以为空!', {icon: 5})
            return;
        }
        if ($('#otherTimeOther').val() == '') {
            layer.msg('获得日期不可以为空!', {icon: 5})
            return;
        }
        if ($('#belongAcademyOther').val() == '') {
...
function saveInfo() {
    var params = {
        cnName: $('#cnNameOther').val().trim()
    }
}

function init_add_modal_select() {
    init_select('achieveTypeOther', selectClassifyTypeUrl, {pidName: pidName}, pidName);
    init_select('authorRankOther', selectAuthorRankUrl, {pidName: pidName}, null);
    init_select('belongAcademyOther', selectAcademyAllUrl, null, userAcademyName);

}

function init_edit_modal_select(hrs) {
    init_select('achieveTypeOther', selectClassifyTypeUrl, {pidName: pidName}, hrs[0].achieveType);
    init_select('authorRankOther', selectAuthorRankUrl, {pidName: pidName}, hrs[0].authorRank);
    init_select('belongAcademyOther', selectAcademyAllUrl, null, hrs[0].belongAcademy);

}

function init_edit_modal_input(hrs) {
    $('#cnNameOther').val(hrs[0].cnName);
    $('#other-id').val(hrs[0].id);
    $('#otherTimeOther').val(hrs[0].otherTime);
    $('#chieveInfoOther').val(hrs[0].chieveInfo);
    //TODO 加上传文件 的信息
}
