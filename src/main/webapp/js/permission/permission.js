$mainTable = $('#role-url-table');
mainModal = 'role-url-modal';
mainForm = 'role-url-form';
modalTitle = 'roleUrlModelTitle';
so.init(function () {
    // 全选
    roleTable();
    ...
    $('#btn-role-save').click(function () {
        if ($('#role-name').val().trim() == '') {
            layer.msg('权限名称不可以为空', {icon: 2});
            return;
        }
        if ($('#role-url').val().trim() == '') {
            layer.msg('权限路径不可以为空', {icon: 2});
            return;
        }

        params = {id: $('#role-id').val(), name: $('#role-name').val().trim(), url: $('#role-url').val().trim()}
    ...
    $('#' + mainModal).on('show.bs.modal', function () {
        ...
        if ($('#' + modalTitle).html().length == 2) {
            $('#' + modalTitle).html($('#' + modalTitle).html() + "权限路径");
        }
