...
var uploadFile = "attachNameStudents";
$(function(){
    initeTable();
    init_select('academy', selectAcademyAllUrl, null, null);
    initeOnFunction();
    initFileInput(uploadFile,fileUplaodUrl,mainModal);
    $('#'+main.modal).on('shown.bs.modal', function () {

        if($('#'+main.title).html().length==2) {
            $('#'+main.title).html($('#studentModelTitle').html() + "学生");
        }
    ...
    $('#btn-modal-save').click(function () {
        ...
        if (isNullUndefineEmpty($('#majorName').val())) {
            layer.msg('专业不可以为空!', {icon: 5})
            return;
        }
...
function init_add_modal_select() {
    init_select('academyName', selectAcademyAllUrl, null, $('#academyName').find('option:selected').text());

}

function init_edit_modal_select(hrs) {
    init_select('academyName', selectAcademyAllUrl, null, hrs[0].academyName);
    init_select('majorName', selectMajorDataUrl,{academy:hrs[0].academyName}, hrs[0].majorName);
}

function init_edit_modal_input(hrs) {
    $('#name').val(hrs[0].name);
    $('#student-id').val(hrs[0].id);
    $('#modal-number').val(hrs[0].number);
    //$('#majorName').val(hrs[0].majorName);
    $('#sex').val(hrs[0].sex);
...
function initeOnFunction() {
    $('#academy').on('rendered.bs.select', function (e) {
        ...
        init_select('major', selectMajorDataUrl, {academy:academy}, null);
    ...
    $('#academyName').on('rendered.bs.select', function (e) {
        var academy = $(e.currentTarget).find('option:selected').text();
        //init_select('majorName', selectMajorDataUrl, {academy:academy}, hrs[0].majorName);
    });

    $('#major').on('rendered.bs.select', function (e) {
    ...
    $('#btn-base-print').on('click',function (e) {
        var hrs =$('#'+main.table) .bootstrapTable('getSelections');
        if (hrs.length < 1) {
            layer.msg('请选择一条或多条数据进行删除！', {icon: 2});
            return;
        ...
        layer.confirm('您确定要导出所选择记录吗？', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                ...
                success:function(data,textStatus){
                    if(data.errorMsg=="") {
                        ...
                        layer.alert("以下学生已审核记录为空："+data.errorMsg);
                    }
                    if(data.fileMsg!=""){
                        fileNames = data.fileMsg.substring(0,data.fileMsg.length-1).split('_');
                        for(var i=0;i<fileNames.length;i++) {
                            window.open(baseUrl + "static/upload/" + fileNames[i], '_blank');
                        }
                    }
